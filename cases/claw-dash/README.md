...
이 문서는 프로젝트 자체의 자랑이 아니라, [워크스페이스 README](../../README.md)에서 설명한 **살아있는 개발 표준 시스템이 실제 프로젝트에서 어떻게 작동했나의 추적**이다. 컨텍스트를 어떻게 구성했고, AI와 어디서 헤맸으며, 표준의 어느 부분을 그대로 따랐고 어디를 의도적으로 예외 처리했는지 — 그 흐름이 핵심이다. claw-dash는 BFF 특성상 표준 일부(예: Response Envelope)가 의도적으로 적용되지 않는 예외 사례이기도 하다.
...
작업 사이클은 **설계 문서 작성 → 구현 플랜 생성 → 단계별 실행**을 반복했다.

역할 분담은 아래와 같다.

| 역할 | 담당 |
|------|------|
| 기능 요구사항 정의, 설계 방향 결정 | 나 |
| 트레이드오프 검토 및 설계 승인 | 나 |
| 설계 문서 초안 작성 | AI (Claude Code) |
| 구현 플랜 생성 및 단계별 실행 | AI (Claude Code) |
| 코드 리뷰, 동작 검증, 회귀 확인 | 나 |
| 예외 판단 및 설계 변경 결정 | 나 | 백엔드 구조 리팩터링처럼 영향 범위가 큰 작업은 반드시 디자인 문서를 먼저 만들고 승인한 뒤 실행했다 ([backend-structure-refactor-design](./plans/2026-03-20-backend-structure-refactor-design.md)). 칸반 안정화처럼 복잡한 도메인은 claim matrix로 현재 상태를 사실/가설로 분류하고, phase별 scope lock을 걸어 의미 변경과 구조 변경이 섞이지 않도록 제어했다 ([kanban-stabilization-plan](./plans/2026-03-24-kanban-stabilization-plan.md)). 프로젝트 전체에서 50개 이상의 플랜 문서가 생성됐다. 이 중 대표적인 문서를 [`plans/`](./plans/)에 공개했다.
...
- 플랜 문서 50개 이상 생성 (이 중 대표적인 문서를 `plans/`에 공개)
