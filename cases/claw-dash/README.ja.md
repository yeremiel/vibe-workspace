...
このドキュメントはプロジェクト自慢ではなく、[ワークスペース README](../../README.ja.md) で説明した**生きた開発標準システムが実プロジェクトでどう機能したかの追跡**だ。コンテキストをどう構成し、AIとどこで試行錯誤し、標準のどの部分はそのまま守り、どこを意図的に例外として扱ったか — その流れが核心となる。claw-dashはBFFの特性上、標準の一部（例: Response Envelope）が意図的に適用されない例外事例でもある。
