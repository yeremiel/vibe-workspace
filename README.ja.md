...
3. **[参考]** [environment/CLAUDE.ja.md](environment/CLAUDE.ja.md) — AIコンテキスト設計の原本
...
構造設計の詳細な理由 → [environment/workspace-structure.ja.md](environment/workspace-structure.ja.md)
...
claw-dash以外に、同じ方法で進めているプロジェクトだ。コードは非公開だが、同一のAI協働環境を使用している。

- **myLib** — 個人蔵書管理サービス。Go/Ginバックエンド + Flutterフロントエンドのフルスタック。将来的に自分のサービスとしてリリース予定
- **scanner** — Flutter + Android CameraX + OpenCVベースのオフライン文書スキャナー
